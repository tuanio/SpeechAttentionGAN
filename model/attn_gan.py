...
class MagnitudeAttentionGAN(L.LightningModule):
    ...
    def on_train_epoch_end(self):
        ...
        mag_A = torch.stack([i[0] for i in self.training_output], dim=0).type_as(self.gen_A2B[0].weight)
        mag_B = torch.stack([i[1] for i in self.training_output], dim=0).type_as(self.gen_A2B[0].weight)
